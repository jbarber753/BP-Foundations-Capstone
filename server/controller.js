...
module.exports = {
    ...
    getKickerStats: (req, res) => {
        ...
        if (!stat && !sort){
            sequelize
            .query(`SELECT SETSEED(0.477); SELECT CONCAT(t.location,' ', t.name) AS team, k.id, p.name, k.xp_attempts, FLOOR(RANDOM() * (xp_attempts - (xp_attempts / 2)) + (xp_attempts / 2)) AS xp_made, k.fg_attempts, FLOOR(RANDOM() * (fg_attempts - (fg_attempts / 2)) + (fg_attempts / 2)) AS fg_made FROM kickers k JOIN players p ON k.id = p.player_id JOIN teams t ON p.team = t.team_id`)
            .then(dbres => {
                res.status(200).send(dbres[0])
            })
            .catch(error => console.log(error))
