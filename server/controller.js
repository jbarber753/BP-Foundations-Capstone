...
module.exports = {
    ...
    getKickerStats: (req, res) => {
        ...
        if (!stat && !sort){
            sequelize
            .query(`SELECT k.id, p.name, k.xp_attempts, FLOOR(RANDOM() * (xp_attempts - (xp_attempts / 2)) + (xp_attempts / 2)) AS xp_made, k.fg_attempts, FLOOR(RANDOM() * (fg_attempts - (fg_attempts / 2)) + (fg_attempts / 2)) AS fg_made FROM kickers k JOIN players p ON k.id = p.player_id`)
            .then(dbres => {
                res.status(200).send(dbres[0])
            })
            .catch(error => console.log(error))
            ...
            sequelize
            .query(`SELECT k.id, p.name, k.xp_attempts, FLOOR(RANDOM() * (xp_attempts - (xp_attempts / 2)) + (xp_attempts / 2)) AS xp_made, k.fg_attempts, FLOOR(RANDOM() * (fg_attempts - (fg_attempts / 2)) + (fg_attempts / 2)) AS fg_made FROM kickers k JOIN players p ON k.id = p.player_id ORDER BY ${stat} ${sort}`)
            .then(dbres => {
                res.status(200).send(dbres[0])
            })
            .catch(error => console.log(error))
